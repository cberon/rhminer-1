...
/*
    /// Create with a nonce at the chosen opposi
    explicit FixedHash(unsigned _u, ConstructFromHashType _t) 
    { 
        size_t usize = sizeof(unsigned)
        if (usize == N)
            memcpy(m_data.data(), &_u, std::min<unsigned>((unsigned)usize, N));
        else
        {
            m_data.fill(0);
            RHMINER_ASSERT(_t != FailIfDifferent);
            byte* _b = (byte*)&_u;
            auto c = std::min<unsigned>((unsigned)usize, N);
            for (unsigned i = 0; i < c; ++i)
                m_data[_t == AlignRight ? N - 1 - i : i] = _b[_t == AlignRight ? usize - 1 - i : i];
        }
    }
*/
    ...
    { 
        if (_b.size() == N) 
            memcpy(m_data.data(), _b.data(), std::min<unsigned>((unsigned)_b.size(), N));
        else 
        {
            m_data.fill(0); 
            if (_t != FailIfDifferent) 
            { 
                auto c = std::min<unsigned>((unsigned)_b.size(), N);
                for (unsigned i = 0; i < c; ++i) 
    ...
    void CopyFrom(const int targetIndx, byte const* sourceData, const int sourceSize)
    {
        RHMINER_ASSERT(targetIndx + sourceSize < N);
        memcpy(m_data.data() + targetIndx, sourceData, sourceSize);
    }

    void CopyFromString(const int targetIndx, const string& srcHex)
    {
        bytes hashBin = fromHex(srcHex, WhenError::Throw);
        RHMINER_ASSERT(targetIndx + hashBin.size() < N);
        memcpy(m_data.data() + targetIndx, hashBin.data(), hashBin.size());
    }

    void CopyTo(bytes& targetArray, const int targetIndx) const
    {
        auto res = targetIndx + m_data.size();
        if (res > targetArray.size())
            targetArray.resize(res);

        memcpy(targetArray.data() + targetIndx, m_data.data(), m_data.size());
    }
    ...
    bool isValid() const { return std::any_of(m_data.begin(), m_data.end(), [](byte _b) { return _b != 0; }); }
...
inline bool IsHashLessThan_32(const void* op1, const void* op2)
{
    uint32_t *hash32 = (uint32_t *)op1;
    uint32_t *target32 = (uint32_t *)op2;
    bool isless = true;
...
template <unsigned N>
inline bool IsHashLessThan(const FixedHash<N> op1, const FixedHash<N>& op2)
{
    return IsHashLessThan_32((const void*)op1.data(), (const void*)op2.data());
}


template <unsigned N>
inline bool IsHashLessThanBE(const FixedHash<N> op1, const FixedHash<N>& op2)
{
    uint32_t *hash32 = (uint32_t *)op1.data();
    uint32_t *target32 = (uint32_t *)op2.data();
    bool isless = true;

    for (int i = 0; i < 32; i+=4)
    {
        uint32_t h32tmp = hash32[i];
        uint32_t t32tmp = RH_swap_u32(target32[i]);
...
namespace std
{
	/// Forward std::hash<FixedHash> to FixedHash::hash.
	template<> struct hash<h64>: h64::hash {};
	template<> struct hash<h128>: h128::hash {};
	template<> struct hash<h160>: h160::hash {};
	template<> struct hash<h256>: h256::hash {};
	template<> struct hash<h512>: h512::hash {};
}
